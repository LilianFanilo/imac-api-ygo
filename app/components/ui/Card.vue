...
<template>
    <NuxtLink :to="'/Cards/'+ card.id">
        <div class="grid grid-rows-20 w-80 h-160 bg-white rounded px-4 py-2 outline-4 outline-transparent hover:outline-sky-500 transition-all duration-100">
            ...
            <div class="row-span-1">
                <ul class="text-xs flex gap-x-4">
                    <li class="border rounded-full px-2 py-1">{{ card.type }}</li>
                    <li class="border rounded-full px-2 py-1">{{ card.race }}</li>
                    <li class="border rounded-full px-2 py-1" v-if="card.level">Level {{ card.level }}</li>
                </ul>
            </div>
