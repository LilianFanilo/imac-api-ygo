  ...
  <NuxtLink :to="'/Cards/' + card.id">
    ...
    >
      <div class="flex flex-col gap-y-2.5">
        ...
        ></div>
        <ul class="text-xs flex flex-wrap gap-1">
          <li
            class="flex w-fit h-fit items-center border rounded-full px-2 py-1"
          >
            {{ card.type }}
          </li>
          <li
        ...
        <p class="max-h-36 text-xs text-justify line-clamp-9">
          {{ card.desc }}
        </p>
      </div>
      <div
        v-if="card.atk && card.def"
        class="row-start-20 text-end"
      >
        ATK/{{ card.atk }} DEF/{{ card.def }}
      </div>
      <div
        v-else-if="card.atk && card.linkval"
        class="row-start-20 text-end"
      >
